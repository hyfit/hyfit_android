...
class GoalRVAdapter(val context: Context, val result: ArrayList<Goal>): RecyclerView.Adapter<GoalRVAdapter.ViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): GoalRVAdapter.ViewHolder {
        // rv_item의 textView 불러오기

        val binding: GoalListBinding = GoalListBinding.inflate(LayoutInflater.from(parent.context), parent, false)

        return ViewHolder(binding)
    }


    override fun onBindViewHolder(holder: GoalRVAdapter.ViewHolder, position: Int) {
    ...
    inner class ViewHolder(val binding: GoalListBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    fun changeItem(goal : Goal){
        val newBinding =GoalDetailListBinding.inflate(LayoutInflater.from(context), null, false)
        // 바인딩할 데이터 설정
        newBinding.goalRate.text = goal.rate.toString() + "%"
        newBinding.goalDescription.text = goal.description
        // 변경된 데이터를 뷰에 반영
